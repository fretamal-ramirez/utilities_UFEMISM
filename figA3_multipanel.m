...
for i = 1:nCols
    ...
    Hi_diff(:,i) = mesh.Hi(:,end) - mesh.Hi(:,1);
    mesh.Hb_diff = mesh.Hb(:,end) - mesh.Hb(:,1);
    uabs_diff(:,i) = mesh.uabs(:,end) - mesh.uabs(:,1);
    [~, maskHi_ROI] = Hi0_to_NaN_mesh(Hi_diff(:,i));
    ...
    %if i <=3
        plot_mesh_data_b_RLIS(mesh, uabs_diff(:,i).*mask_vel, ax_all(1,i),plot_font_size);
    %else
    ...
    ax_all(2,i) = nexttile(nCols + i); % row 2
    %if i<=3
        plot_mesh_data_a_RLIS(mesh, Hi_diff(:,i).*maskHi_ROI, ax_all(2,i),plot_font_size);
    %else
...
print(fig,'/Users/frre9931/Documents/PhD/ANT_UFEMISM/plots_ant/Riiser-Larsen/multipanel/multipanel_Hb_OC+RT50_all.pdf','-dpdf','-vector');
